    ...
    <div className="flex min-h-[100dvh] w-full bg-slate-50 dark:bg-slate-950">
      <aside className="hidden md:flex flex-col w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 fixed inset-y-0 z-50">
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center text-white font-bold shadow-sm">B</div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const itemActive = location.startsWith(item.href);
            ...
            return (
              ...
              >
                <Icon className={cn('w-5 h-5 flex-shrink-0', isAI && !itemActive && 'text-emerald-600 dark:text-emerald-400')} />
                {item.label}
              </Link>
            );
          })}
