# Migrating Expo UI to Blue Collar Financial AI Design System

Read `artifacts/bcfai-ds/docs/AGENTS.md` and
`artifacts/bcfai-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/bcfai-ds/lib/native-theme` and
  `@workspace/bcfai-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/bcfai-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/bcfai-ds` and retains only
