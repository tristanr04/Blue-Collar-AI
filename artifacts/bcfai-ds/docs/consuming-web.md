# Consuming Blue Collar Financial AI Design System in web apps

Read `artifacts/bcfai-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/bcfai-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/bcfai-ds/styles.css";
```
...
`node_modules/@workspace/bcfai-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/bcfai-ds/components/ui/button";
import { cn } from "@workspace/bcfai-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/bcfai-ds/hooks/use-toast";
```
...
`@workspace/bcfai-ds/components/ui/button`. Run the app's typecheck
